...
async def try_on(session, model_img, dress_img):
    ...
    async with session.post(API_URL, json=data, headers=headers) as res:
        ...
        if res.status != 200:
            ...
            return {"status": "failed", "error": f"Failed to create task. Status: {res.status}, Response: {error_text}"}
        res_json = await res.json()
        task_id = res_json['data']['task_id']

    check_url = f"{API_URL}/{task_id}"
    status = "pending"
    check_json = None  # Define check_json outside the loop
    ...
    while status in ["pending", "running", "processing"]:
        ...
        async with session.get(check_url, headers=headers) as check_res:
            if check_res.status != 200:
                status = "failed"
                check_json = {"data": {"status": "failed", "error": "Could not get task status."}}
                break
            check_json = await check_res.json()
            status = check_json['data']['status']

    # --- THIS IS THE FIX ---
    # Check if the final status is 'completed' before trying to access the output.
    if status == 'completed' and check_json.get('data', {}).get('output'):
        try:
            ...
            # This catches errors if the successful response structure is unexpected
            return {"task_id": task_id, "status": "failed", "error": f"Could not parse successful output: {str(e)}"}
    else:
        # Handle the case where the task failed on the external API's side
        api_error = check_json.get('data', {}).get('error', 'Unknown error from external API.')
        return {"task_id": task_id, "status": "failed", "error": api_error}
